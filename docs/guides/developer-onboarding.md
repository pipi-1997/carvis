...
- 公开 npm 发布默认通过 trusted publishing 完成，不要为 CI 新增或回填长期 `NPM_TOKEN`
- `@carvis/carvis-media-cli` 当前是内部 transport CLI，不参与 npm 公开发布，也不属于公开 release group
- 不要手工批量修改多个 `package.json` 版本号来模拟发版
- docs-only、internal-only 或不命中公开 release group 的改动通常不需要 changeset
- 若你在维护 AI 工具入口、prompt、command、rule 或 agent 指导文件，release 规则必须同步到所有现有入口
- 本地可选使用 `gh` 查看 release PR / workflow / release 状态，但它不是 CI 主流程依赖
